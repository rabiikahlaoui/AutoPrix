...
def product(url):
    try:
        ...
        if(len(web_soup.findAll("div",{"class":"infos"})[0].findAll("li")[0].findAll("span"))>0):
...
def page_scap(url):
    ...
    for i in web_soup.findAll("a"):
        ...
        if not link.startswith("http"):
            if link.startswith("/"):
                link = "https://www.automobile.tn" + link
            else:
                link = url + link
        if link.startswith("https://www.automobile.tn") and not link.endswith("#"):
            if not link in links:
                links.append(link)
                subs = link.replace('https://www.automobile.tn/fr/occasion/','')
                if subs.isdigit():
                    print("scaping : " + link)
                    page_scap(link)
                    sleepF()
                elif link.startswith("https://www.automobile.tn/fr/occasion/") and not link in skipLinks:
                    if link == "https://www.automobile.tn/fr/occasion/vendeurs-pro/tunisie-auto-prix":
                        page_scap(link)
                    else:
                        print("product : " + link)
                        product(link)
...
page_scap("https://www.automobile.tn/fr/occasion")
